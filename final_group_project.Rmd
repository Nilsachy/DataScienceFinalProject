...
## Split trans_date_trans_time to year, month, date, hour, minute, and second

```{r}
card_fraud <- card_fraud %>% 
  mutate(trans_date_trans_time = as.POSIXct(trans_date_trans_time , format = "%Y-%m-%d %H:%M:%S"),
         trans_year=as.numeric(format(trans_date_trans_time , "%Y")),
         trans_month = as.numeric(format(trans_date_trans_time , "%m")),
         trans_day = as.numeric(format(trans_date_trans_time , "%d")),
         trans_hour = as.numeric(format(trans_date_trans_time , "%H")),
         trans_minute = as.numeric(format(trans_date_trans_time , "%M")),
         trans_second = as.numeric(format(trans_date_trans_time , "%S")))
...
```{r}
card_fraud <- card_fraud %>%
    mutate(weekday=as.Date(trans_date_trans_time, format="%Y-%m-%d %H:%M:%S"),
           weekday=weekdays(weekday),
           age=round(as.numeric(trans_date_trans_time - dob) / 365.25))
```

## Convert categorical and non-ordinal numerical variables to factors

```{r}
card_fraud$trans_year <- factor(card_fraud$trans_year)
card_fraud$trans_month <- factor(card_fraud$trans_month)
card_fraud$trans_hour <- factor(card_fraud$trans_hour)
card_fraud$trans_minute <- factor(card_fraud$trans_minute)
card_fraud$trans_second <- factor(card_fraud$trans_second)
...
card_fraud$weekday <- factor(card_fraud$weekday)
```
...
df_fraud_over_city <- card_fraud %>%
  group_by(state, city) %>%
  summarize(
    lon = mean(long),
    lat = mean(lat),
    num_of_fraud = sum(is_fraud==1),
    num_of_non_fraud=sum(is_fraud==0)
  )

leaflet(data = df_fraud_over_city) %>%
  addTiles() %>%
  addCircleMarkers(
    ~lon, ~lat,
    radius = ~(num_of_fraud / num_of_non_fraud) * 300,
    color = ~colorFactor(rainbow(length(unique(df_fraud_over_city$state))), df_fraud_over_city$state)(state),
    stroke = FALSE, fillOpacity = 0.8,
    popup = ~paste(city)
  ) %>%
  addLegend('bottomright', pal = colorFactor(rainbow(length(unique(df_fraud_over_city$state))), df_fraud_over_city$state), values = ~state, title = 'State', opacity = 1)
```

## Visualize fraud over weekday

```{r}
df_fraud_over_weekday <- card_fraud %>%
    group_by(weekday) %>%
    summarize(
      num_of_fraud = sum(is_fraud==1),
      num_of_non_fraud=sum(is_fraud==0)
...
df_fraud_over_weekday <- na.omit(df_fraud_over_weekday)

df_fraud_over_weekday$weekday <- factor(df_fraud_over_weekday$weekday, levels = c('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'), ordered = TRUE)

ggplot(data = df_fraud_over_weekday, aes(x = weekday, y = num_of_fraud/num_of_non_fraud)) +
    geom_col() +
...
```{r}
df_fraud_over_time_of_day <- card_fraud %>%
    group_by(trans_hour) %>%
    summarize(
      num_of_fraud = sum(is_fraud==1),
      num_of_non_fraud=sum(is_fraud==0)
...
df_fraud_over_time_of_day <- na.omit(df_fraud_over_time_of_day)

ggplot(data = df_fraud_over_time_of_day, aes(x = trans_hour, y = num_of_fraud/num_of_non_fraud)) +
    geom_col() +
...
df_fraud_over_age <- na.omit(df_fraud_over_age)

ggplot(data = df_fraud_over_age, aes(x = age, y = num_of_fraud/num_of_non_fraud)) +
    geom_point() +
...
```{r}
card_fraud <- card_fraud %>%
    mutate(day_in_the_year=as.Date(trans_date_trans_time, format="%Y-%m-%d %H:%M:%S"),
           day_of_year=yday(trans_date_trans_time)) 

df_fraud_over_trans_date <- card_fraud %>%
    group_by(trans_year, day_of_year) %>%
    summarize(
      num_of_fraud = sum(as.double(is_fraud))
    )

ggplot(data = df_fraud_over_trans_date, aes(x = day_of_year, y = num_of_fraud, color=trans_year)) +
    geom_smooth() +
    labs(x = "Day in the year", y = "Number of fraudulent transactions", title = "Number of fraud over time") +
    theme_minimal()
```
